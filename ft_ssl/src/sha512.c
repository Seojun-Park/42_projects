...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/06/21 12:58:05 by jinpark           #+#    #+#             */
/*   Updated: 2019/06/21 12:58:17 by jinpark          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		sha512_shift(int i, t_sha512 *sha)
{
	...
	while (++index < 16)
	{
		...
		sha->mem[index] = 0;
		while (++j < 8)
			sha->mem[index] += (u_int64_t)sha->msg[i * 128 + index * 8 + j]
				<< (8 * (7 - j));
	}
	index = 15;
	while (++index < 80)
	{
		sp1 = right_rotate_64(sha->mem[index - 15], 1) ^
			right_rotate_64(sha->mem[index - 15], 7) ^
			(sha->mem[index - 15] >> 3);
		sp2 = right_rotate_64(sha->mem[index - 2], 19) ^
			right_rotate_64(sha->mem[index - 2], 61) ^
			(sha->mem[index - 2] >> 6);
...
static void		sha512_set(char *s, t_sha512 *sha)
{
	...
	i = -1;
	while (++i < 8)
		sha->msg[128 * sha->set - i - 1] = len >> (i * 8);
}
